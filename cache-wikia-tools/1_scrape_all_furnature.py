...
def get_items():
    ...
    data = urllib.request.urlopen('http://oldschoolrunescape.wikia.com/wiki/Category:Furniture').read() 
    ...
    i = 2
    while i < 5: # NOTE: This number (29) needs to be MANAUALL CHANGED 
        # For example, if more pages are added
        # It needs to be 1 more than the actual number of pages
        url = 'http://oldschoolrunescape.wikia.com/wiki/Category:Furniture?page=' + str(i)
    ...
    out_fi = open("all_wikia_furnature.txt", 'w')
