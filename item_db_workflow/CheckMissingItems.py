...
class CompareAllitemsFiles(object):
    ...
    def dict_compare(self):
        ...
        added.sort(key=int)
        for id in added:
            if id in normalized_names:
                print("%d|%s|%s" % (int(id), self.allitems_dict[id]["name"], normalized_names[id][2]))
            else:
                print("%d|%s|%s" % (int(id), self.allitems_dict[id]["name"], ""))
...
if __name__=="__main__":
    ...
    with open(".." + os.sep + "wiki_extraction_tools" + os.sep + "normalized_names.txt") as f:
        for l in f:
            l = l.strip()
            if "#" in l or l.startswith("TODO"):
                continue
            l = l.split("|")
            normalized_names[l[0]] = l
